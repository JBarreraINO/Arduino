  ...
  unsigned char emitterPin)
{
    ...
    if (numSensors > QTR_MAX_SENSORS)
        _numSensors = QTR_MAX_SENSORS; // 16 sensores MAXIMO
    else
...
// Lee los valores del sensor en una matriz. Allí * DEBE * *MUST* ser espacio
// para tantos valores como sensores especificados en el constructor.
// Ejemplo de uso:
// unsigned int sensor_values ​​[8];
// sensores.read (sensor_valores);
// Los valores devueltos son una medida de la reflectancia en unidades abstractas,
// con valores más altos correspondientes a una menor reflectancia (por ejemplo, un negro
// superficie o un vacío).
...
void QTRSensors::saveCalibrationEEPROM (){
    const int ns = _numSensors;
    struct calibratedValues{
        unsigned int caliMin[_numSensors];
        unsigned int caliMax[_numSensors];
    };

    calibratedValues values;
    for (int i=0; i<_numSensors; i++){
        values.caliMin [i] = calibratedMinimumOn[i];
        values.caliMax [i] = calibratedMaximumOn[i];
    }
    int eeAdress = sizeof (calibratedValues);
    EEPROM.put (eeAdress, values);
}

void QTRSensors::readCalibrationEEPROM (){
    const int ns = _numSensors;
    struct calibratedValues{
        unsigned int caliMin[_numSensors];
        unsigned int caliMax[_numSensors];
    ...
    int eeAdress = sizeof (calibratedValues);
   calibratedValues values;
   EEPROM.get(eeAdress, values);
   for (int i=0; i<_numSensors; i++){
    calibratedMinimumOn[i]=values.caliMin [i];
    calibratedMaximumOn[i]=values.caliMax [i];
  }
}
...
void QTRSensors::readCalibrated(unsigned int *sensor_values, unsigned char readMode,int flanco_sens)
{
    ...
    for(i=0;i<_numSensors;i++)
    {
        ...
        */
        signed int x = 0;
        if (denominator!= 0){
        	x = constrain (map (sensor_values[i], calmin, calmax, 0, 1000), 0, 1000);
        }
    ...
    unsigned char emitterPin)
{
    ...
    _maxValue = 4095; // this is the maximum returned by the A/D conversion in ESP32
}
...
QTRSensorsMux::QTRSensorsMux(unsigned char* pins, unsigned char numSensors, unsigned char numSamplesPerSensor,
                            unsigned char inPin, unsigned char* muxPins)
{
    calibratedMinimumOn = 0;
    calibratedMaximumOn = 0;
    calibratedMinimumOff = 0;
    ...
    init(pins, numSensors, numSamplesPerSensor, inPin, muxPins);
}


void QTRSensorsMux::init (unsigned char* pins, unsigned char numSensors, unsigned char numSamplesPerSensor,
                            unsigned char inPin, unsigned char* muxPins)
{
    QTRSensors::init(pins, numSensors, (unsigned char)255);
    unsigned char i;
    for (i = 0; i < 4; i++)
    {
...
void QTRSensorsMux::readPrivate(unsigned int *sensor_values)
{
    unsigned char i, j;
    ...
    for (j = 0; j < _numSamplesPerSensor; j++)
    {
        for (i = 0; i < _numSensors; i++)
        {
            sensor_values[i] += readMux(_pins[i]);
        }
    }
...
unsigned int QTRSensorsMux::readMux (unsigned char channel){
    ...
    unsigned int muxValue = analogRead(_inPin);
    return muxValue;
}
