...
//////*********************
  ...
  private:
    ...
    void initializeCalibrationVectors (unsigned int **calibratedMinimum, unsigned int **calibratedMaximum);
...
class QTRSensorsMux : public QTRSensors
{
  public:
    ...
    // this constructor just calls init()
    QTRSensorsMux(unsigned char* pins,
  ...
  private:
    ...
    // reflectance (e.g. a black surface or a void).
    void readPrivate(unsigned int *sensor_values);
    unsigned int readMux (unsigned char channel);
    unsigned char _numSamplesPerSensor;
    unsigned char _muxPins[4];
    unsigned char _inPin;
};
