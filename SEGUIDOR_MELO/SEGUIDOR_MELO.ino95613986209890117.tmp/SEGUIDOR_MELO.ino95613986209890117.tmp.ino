...
int reversa = 50;  // Velocidad a la que ira el motor hacia atras cuando pierda la linea    max 255
...
//Si te sales
    ...
    { analogWrite ( pwmd, reversa); digitalWrite ( pind, HIGH);
      analogWrite ( pwmi, frente); digitalWrite ( pini, HIGH); fan.write(120);
    }

    if ( posicion == 1 )
    { analogWrite ( pwmd, frente); digitalWrite ( pind, LOW);
      analogWrite ( pwmi, reversa); digitalWrite (pini, LOW); fan.write(120);
    }
//control de motores 
