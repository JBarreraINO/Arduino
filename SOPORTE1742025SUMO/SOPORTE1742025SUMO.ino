...
int estrategia_de_busqueda;
int estrategiaSeleccionada;

// ULTIMO SENSOR
int ultimo_sensor = -1;  // -1: ninguno, 0: derecha, 1: izquierda
unsigned long tiempo_ultimo_sensor = 0;
const unsigned long tiempo_max_memoria = 100;  // 1 segundo de memoria activa
//
bool confirmando_lateral = false;
byte lado_confirmacion = 255;  // 0: derecho, 1: izquierdo
unsigned long tiempo_confirmacion = 0;
const unsigned long duracion_confirmacion = 100;  // ms que “espera” antes de confirmar
  ...
  switch (estrategiaSeleccionada) {
    ...
    case 4:  //izquierda diagonL
      if (tiempo_actual - tiempo_inicio < timeout_str) {
        ...
        controlarMotores(0, 0);
        // MovDes();
        busquedaestrategica(4);
      }
      break;

    case 5:  // Estrategia 6: Buscar borde y retroceder //  OP DE FRENTE 
...
void loop() {
  ...
  if (atacando == false && (estado1x > sens || estado2x > sens)) {
    ...
    if (bandera_estretegia == 1) {
      busquedaestrategica(estrategiaSeleccionada);
    }
  }
}
...
void busquedaestrategica(int estrategia) {
  unsigned long ahora = millis();

  switch (estrategia) {

    case 0:
      {  // Estrategia 0: barrido estrecho derecha-izquierda
MovDes();
        break;
      }

    case 1:
      {  busquedaestrategica2();
      }
    ...
    case 3:
      {  // Estrategia 3: espiral creciente (derecha)
        static unsigned long t3 = 0;
        static unsigned long paso3 = timeout_busqueda;
        if (ahora - t3 >= paso3) {
          t3 = ahora;
          paso3 += timeout_busqueda / 5;  // incrementa el arco
        }
        DerechaLento();
        break;
      }
    ...
    case 5:
      {  // Estrategia 5: ir esquina derecha
            static bool primero = true;
        if (primero) {
          retornar();  // vuelve al centro
          primero = false;
        }
        DerechaLento();
        break;
    ...
    case 6:
      {  // Estrategia 6: búsqueda coherente aleatoria
        static unsigned long t6 = 0;
        static bool haciaDerecha = true;
        if (ahora - t6 >= timeout_busqueda) {
          t6 = ahora;
          haciaDerecha = random(0, 2);
        }
        if (haciaDerecha) DerechaLento();
        else IzquierdaLento();
        break;
      }

    case 7:
      {  // Estrategia 7: volver al centro + giro derecho
        static bool primero = true;
        if (primero) {
          retornar();  // vuelve al centro
          primero = false;
...
void busquedaestrategica2() {
  ...
  // Verifica si ha pasado el tiempo necesario para cambiar de estado
  if (tiempoActual - tiempoAnterior >= timeout_busqueda) {
    tiempoAnterior = tiempoActual;  // Actualiza el tiempo de la última transición
    avanzando++;                    // Cambia al siguiente estado
