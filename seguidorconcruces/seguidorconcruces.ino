...
Servo fan;
int velocidad =76; // Velocidad crucero de robot max 255

float Kp = 0.35; // calibracion proporciona p = 0.145;
float Kd = 2.9;  //  calibracion derivativod = 1.9;
float Ki=0.003;
int sensibilidad = 250;
int frente = 90;   // Velocidad a la que ira el motor hacia adelante cuando pierda la linea max 255  198 para sensores cortos
int reversa = 70;  // Velocidad a la que ira el motor hacia atras cuando pierda la linea    max 255  198 para sensores cortos
...
int pini = 15;//cambie 9 * 10 para direccion
...
PD:
  ...
  if (ACTUAL >= MUESTREO)
  {
    ...
    ERROR_POSICION =  valor_sensor[1] * (-1)  + valor_sensor[3] * (-.8) + valor_sensor[5] * (-.4)  + valor_sensor[7] * (-.1) + valor_sensor[8] * (.1)  + valor_sensor[10] * (.9) +  valor_sensor[12] * (1.3) +  valor_sensor[14] * (2.3) ;
  ...
  error1=ERROR_POSICION;
    ...
    if ( valor_sensor[3] > sensibilidad | valor_sensor[12] > sensibilidad ) {
      posicion = 0;//en rango
    }
    ...
    if ( valor_sensor[14]> sensibilidad && valor_sensor[12] > sensibilidad   ) {
      posicion = 16;//sale hacia izquierda
    }
