...
void loop() {
 ...
 { 
   tft.fillScreen(0x0000);delay(1500);
  termo(); 
...
int ringMeter(int value, int vmin, int vmax, int x, int y, int r, char *units, byte scheme)
{
 ...
 float resultado;
 resultado=((mlx.readObjectTempC()+8.7)+36.5)/2;
...
float  resultado=((mlx.readObjectTempC()+8.7)+36.5)/2;
...
tft.setRotation(1); 
  ...
  ringMeter(resultado,35,41, xpos,ypos,radius,"Grados",BLUE2RED); // metro analogo 
...
//Design Interface (lines)
 ...
 if (resultado >= 37.6 && resultado <=39){ 
...
if(resultado >= 33.5 && resultado<=37.5)
{  tft.setCursor (2,220);//Small celcius text
    tft.setTextSize (2);
