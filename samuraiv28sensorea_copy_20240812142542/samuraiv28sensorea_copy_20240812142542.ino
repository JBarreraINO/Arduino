...
#include<Servo.h>
Servo fan;
int velocidad =80; // Velocidad crucero de robot max 255
float Kp = 0.4; // calibracion proporciona p = 0.145;
float Kd = 2.5;  //  calibracion derivativod = 1.9;
float Ki = 0.003;
int sensibilidad = 100;  //
int frente = 60;   // Velocidad a la que ira el motor hacia adelante cuando pierda la linea max 255  198 para sensores cortos
int reversa = 60;  // Velocidad a la que ira el motor hacia atras cuando pierda la linea    max 255  198 para sensores cortos
...
PD:
  ...
  if (ACTUAL >= MUESTREO)
  {
    ...
    ERROR_POSICION =  valor_sensor[1] * (-1)  + valor_sensor[3] * (-.8) + valor_sensor[5] * (-.4)  + valor_sensor[7] * (-.1) + valor_sensor[8] * (.1)  + valor_sensor[10] * (.4) +  valor_sensor[12] * (.8) +  valor_sensor[14] * (1) ;
