...
float Kp=1, Kd=1, Ki=0.001;  //constantes
...
Leer_Sensores_ir(0);
 //pid(0, 120, 0.18, 0.001, 4 );
  pid(0,velocidad,Kp,Ki,Kd); //funcion para algoritmo pid 
  ...
  frenos_contorno(linea,5); //funcion para frenado en curvas tipo 
                              //primer parametro :0 para lineas negras, tipo 1 para lineas blancas
...
{
  position = p;//0 para linea negra, 1 para linea blanca
...
if(tipo==0)
{
  if(position<=50) //si se salio por la parte derecha de la linea
 {
  //debido a la inercia, el motor 
       Robot.Speeds (-200,140)  ;     
       turbina(1400);                   
                                
  while(true)  
  {
  for(int i=0; i<=8;i++)  {       //Leo los 16 canales               
    
sensorValues[i] = analogRead(ANALOG_IN); //guardo los valores de los sensores
 ...
 if (position>=14050) //si se salio por la parte izquierda de la linea
 { 
   Robot.Speeds(150,-200);
     turbina(1300);
  while(true)
  {
 for(int i=0; i<=7;i++)  {       //Leo los 16 canales               
