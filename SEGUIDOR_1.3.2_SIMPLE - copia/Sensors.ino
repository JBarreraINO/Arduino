...
void sensorsInit() {
  ...
  //  set AVcc as ADC voltage reference and right adjusted bit and channel
  ...
  //  set ADC channel
  ADMUX = (ADMUX & ~ADC_CHANNEL_MASK) | 4;
}

void readRawSensors() {
  //  sensor reading loop
  for (byte i = 0; i < TOTAL_SENSORS; i++) {
    PORTC = (PORTC & ~MUX_SWITCH_MASK) | i;  //  set MUX switch output configuration

    ADCSRA |= (1 << ADEN) | (1 << ADSC);  //  enable ADC and start conversion
    while (bit_is_set(ADCSRA, ADSC))      //  wait for conversion
      ;
    //sensorValues[i] = ADCH;  //  storage ADC conversion
     sensorValues[i] = ADCH;
